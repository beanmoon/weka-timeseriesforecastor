...
public class ForecastingPanel extends JPanel {
	...
	/**
	 * The split panel to divided the history/results area from the
	 * configuration
	 */
			...
			try {
				...
				if (m_threadForecaster instanceof OverlayForecaster
					...
					if (!eval.getEvaluateOnTestData()
						...
						// warn the user that future forecast can't be produced
				...
				if (m_advancedConfigPanel.getOutputPredictionsAtStep() > 0) {
					...
					if (eval.getTrainingData() != null && eval.getEvaluateOnTrainingData()) {
						String predString = eval.printPredictionsForTrainingData("=== Predictions "
								...
								+ (step > 1 ? "-steps ahead)" : "-step ahead)") + " ===", targetName, step,
								eval.getPrimeWindowSize());
						outBuff.append("\n").append(predString);
					}

					if (eval.getTestData() != null) {
						int instanceNumOffset = (eval.getTrainingData() != null && m_advancedConfigPanel
								.getHoldoutSetSize() > 0) ? eval.getTrainingData().numInstances() : 0;

						String predString = eval.printPredictionsForTestData("=== Predictions " + "for test data: "
								+ targetName + " (" + step + (step > 1 ? "-steps ahead)" : "-step ahead)") + " ===",
				...
				if (m_advancedConfigPanel.getGraphPredictionsAtStep() > 0) {
					...
					if (eval.getTrainingData() != null && eval.getEvaluateOnTrainingData()) {
						JPanel trainTargetsAtStep = eval.graphPredictionsForTargetsOnTraining(
								GraphDriver.getDefaultDriver(), m_threadForecaster, targets, stepNum,
								eval.getPrimeWindowSize());
					...
					if (eval.getTestData() != null && eval.getEvaluateOnTestData()) {
						...
						JPanel testTargetsAtStep = eval.graphPredictionsForTargetsOnTesting(
								GraphDriver.getDefaultDriver(), m_threadForecaster, targets, stepNum, instanceOffset);
				...
				if (m_advancedConfigPanel.getGraphTargetForSteps()) {
					...
					if (!fieldsToForecast.contains(selectedTarget)) {
						throw new Exception("Cannot graph predictions for \"" + selectedTarget
								+ "\" because that field is not being predicted.");
					}
					List<Integer> stepList = m_advancedConfigPanel.getGraphTargetForStepsStepList();

					if (eval.getTrainingData() != null && eval.getEvaluateOnTrainingData()) {
						JPanel trainStepsForTarget = eval.graphPredictionsForStepsOnTraining(
								GraphDriver.getDefaultDriver(), m_threadForecaster, selectedTarget, stepList,
								eval.getPrimeWindowSize());
				...
				if (m_advancedConfigPanel.getGraphFuturePredictions()) {
					if (eval.getTrainingData() != null /*
														 ...
														 */) {
						try {
							JPanel trainFuture = eval.graphFutureForecastOnTraining(GraphDriver.getDefaultDriver(),
									m_threadForecaster,
							...
							if (m_threadForecaster instanceof OverlayForecaster
								...
								if (m_log != null) {
									m_log.logMessage("Unable to graph future forecast for " + "training data: "
											+ ex.getMessage());
								}
							}
						}
					...
					if (eval.getTestData() != null && eval.getEvaluateOnTestData()) {
						try {
							JPanel testFuture = eval.graphFutureForecastOnTesting(GraphDriver.getDefaultDriver(),
									m_threadForecaster,
									AbstractForecaster.stringToList(m_threadForecaster.getFieldsToForecast()));
							...
							if (m_threadForecaster instanceof OverlayForecaster
								...
								if (m_log != null) {
									m_log.logMessage("Unable to graph future forecast for test " + "data: "
											+ ex.getMessage());
								}
							}
						}
				...
				try {
					if (m_configAndBuild) {
						WekaForecaster copiedForecaster = (WekaForecaster) AbstractForecaster
								.makeCopy(m_threadForecaster);
				...
				JOptionPane.showMessageDialog(ForecastingPanel.this,
						"Problem evaluating forecaster:\n" + ex.getMessage(), "Evaluate forecaster",
	 ...
	 * 
	 * @param log
	 *            the log to use (may be null for no log)
	 * @param displayWelcome
	 *            true if the welcome message is to be displayed as the first
	 *            log entry
	 */
	 ...
	 * @param log
	 *            the log to use (may be null for no log)
	 * @param displayLogWelcome
	 *            true if the welcome message is to be displayed as the first
	 *            log entry
	 * @param allowSeparateTestSet
	 *            true if the separate test set button is to be displayed
	 */
	public ForecastingPanel(LogPanel log, boolean displayLogWelcome, boolean allowSeparateTestSet, boolean kfPerspective) {
		...
		m_configPane.addTab(m_simpleConfigPanel.getTabTitle(), null, m_simpleConfigPanel,
				m_simpleConfigPanel.getTabTitleToolTip());
		m_configPane.addTab(m_advancedConfigPanel.getTabTitle(), null, m_advancedConfigPanel,
				m_advancedConfigPanel.getTabTitleToolTip());
		...
		m_helpBut.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				BrowserHelper.openURL(ForecastingPanel.this, "http://wiki.pentaho.com/display/DATAMINING/"
	 ...
	 * 
	 * @param enable
	 *            true if the start button is to be enabled
	 */
	...
	protected HashMap<String, JTabbedPane> m_framedOutputMap = new HashMap<String, JTabbedPane>();
	...
	protected void resultPopup(final String name, int x, int y) {
		...
		if (selectedName != null && m_runThread == null) {
			...
			reevaluateModelItem
					.setEnabled((m_advancedConfigPanel.m_trainingCheckBox.isSelected() || m_advancedConfigPanel.m_holdoutCheckBox
							.isSelected()) && m_instances != null);
		} else {
	...
	protected void loadForecaster() {
		...
		if (returnVal == m_fileChooser.APPROVE_OPTION) {
			...
			if (!loadOK) {
				...
				JOptionPane.showMessageDialog(this, "Loaded model is not a weka forecaster!", "Weka forecasting",
						JOptionPane.ERROR_MESSAGE);
				loadOK = false;
			}
	 ...
	 * 
	 * @param name
	 *            the name of forecaster to save
	 * @param forecaster
	 *            the actual forecaster to save
	 * @param structure
	 *            the structure of the instances used to train the forecaster
	 */
	 ...
	 * 
	 * @param name
	 *            the name of the entry from which to display results
	 * @param outputPane
	 ...
	 * 
	 * @param entryName
	 *            the entry name of the currently displayed results. If the
	 *            selected result is the same as the current then no update is
	 *            done.
	 */
	...
	protected void sortCheck() {
		...
		if (m_simpleConfigPanel.isUsingANativeTimeStamp()) {
			...
			if (timeStampAtt != null) {
				...
				if (!ok && !hasMissing) {
					...
					int result = JOptionPane.showConfirmDialog(ForecastingPanel.this,
							"The data does not appear to be in sorted order of \"" + timeStampF
									+ "\". Do you want to sort the data?", "Forecasting", JOptionPane.YES_NO_OPTION);
				...
				if (!ok && hasMissing) {
					// we can't really proceed in this situation. We can't sort
					// by the timestamp
					// because instances with missing values will be put at the
					// end of the data.
					// The only thing we can do is to remove the instances with
					// missing time
					// stamps but this is likely to screw up the periodicity and
					// majorly impact
					// on results.
	...
	/**
	 * Inner class defining a log based on PrintStream. This enables command
	 * line, gui and central Weka logging to be unified.
	 * 
	 * @author Mark Hall (mhall{[at]}pentaho{[dot]}com)
	 */
	class LogPrintStream extends PrintStream {
		...
		/**
		 * Log to the status area. Logs to the log area if the string begins
	 ...
	 * 
	 * @param name
	 *            the name of the result from the history list associated with
	 ...
	 */
	protected void reevaluateForecaster(final String name, final WekaForecaster forecaster, final Instances trainHeader) {

		if (!trainHeader.equalHeaders(m_instances)) {
			JOptionPane.showMessageDialog(null,
					"Data used to train this forecaster " + "is not compatible with the currently loaded data:\n\n"
