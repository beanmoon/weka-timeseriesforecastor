...
public class TSLagMaker implements Serializable {
	...
	protected PeriodicityHandler m_dateBasedPeriodicityHandler = new PeriodicityHandler();
	...
	public Enumeration<Option> listOptions() {
		...
		newVector
				.add(new Option("\tSet the minimum lag length to generate." + "\n\t(default = 1)", "L", 1, "-L <num>"));
		newVector
				.add(new Option("\tSet the maximum lag length to generate." + "\n\t(default = 12)", "M", 1, "-M <num>"));
		...
		newVector.add(new Option("\tAdd a day of the week field (requres a date" + " timestamp)", "dayofweek", 0,
				"-dayofweek"));
	 ...
	 * @throws Exception
	 *             if the supplied range is illegal with respect to the min and
	 *             max lag values.
	 */
		...
		if (consecutiveLongLagS.length() > 0) {
			...
			if (consecutive < 1 || consecutive > (getMaxLag() - getMinLag())) {
				throw new Exception("Number of consecutive long lags to average "
						+ "must be greater than 0 and less than " + (getMaxLag() - getMinLag()));
			}
	 ...
	 * 
	 * @param customPeriodic
	 *            the new custom date-derived periodic in textual form.
	 */
	 ...
	 * 
	 * @param names
	 *            a List of field names for which to create lagged variables
	 * @throws Exception
	 *             if a problem occurs
	 */
	 ...
	 * 
	 * @param overlayNames
	 *            the names of the fields that are to be considered "overlay"
	 *            fields
	 */
	 ...
	 * 
	 * @param name
	 *            the name of the time stamp field
	 */
	 ...
	 * 
	 * @param a
	 *            true if we are to adjust for trends via a real or artificial
	 *            time stamp
	 */
	 ...
	 * 
	 * @param v
	 *            true to adjust for variance by taking the log of the
	 *            target(s).
	 */
	 ...
	 * 
	 * @param avg
	 *            true if consecutive long lags are to be averaged.
	 */
	 ...
	 * 
	 * @param p
	 *            the name of the primary periodic attribute (if any) in the
	 *            data.
	 */
	 ...
	 * 
	 * @param am
	 *            true if an AM indicator attribute is to be created.
	 */
	 ...
	 * 
	 * @param d
	 *            true if a num days in month attribute is to be created.
	 */
	 ...
	 * 
	 * @param q
	 *            true if a quarter attribute is to be added.
	 */
	 ...
	 * @throws Exception
	 *             if an artificial time stamp is not being used.
	 */
	 ...
	 * @throws Exception
	 *             if the lag maker is not adjusting for trends or no time stamp
	...
	public double getDeltaTime() {
		return m_dateBasedPeriodicityHandler.deltaTime(); // m_deltaTime;
	}
	...
	public Periodicity getPeriodicity() {
		...
		return m_dateBasedPeriodicityHandler.getPeriodicity();
	}
	...
	private Instances createLags(Instances insts) throws Exception {
		...
		for (int j = 0; j < m_fieldsToLag.size(); j++) {
			...
			for (int i = m_minLag; i <= m_maxLag; i++) {
				 ...
				 * + m_fieldsToLag.get(j)); addE.setExpression("a" + (classIndex
				 * + 1) + "*1"); addE.setInputFormat(insts); insts =
				 * Filter.useFilter(insts, addE); m_lagMakers.add(addE);
				 */
				...
				// the series at 0.
				/*
				 * int count = 0; int lagIndex = insts.numAttributes() - 1; for
				 * (int z = -i; z < 0; z++) { double time = z; double[] vals =
				 * new double[2]; vals[0] = time; vals[1] =
				 * Utils.missingValue(); DenseInstance d = new
				 * DenseInstance(1.0, vals); d.setDataset(lagFillerHeader);
				 * double predictedTarget =
				 * missingLagFiller.classifyInstance(d); if
				 * (insts.instance(count).isMissing(lagIndex)) {
				 * insts.instance(count).setValue(lagIndex, predictedTarget); }
				 * else { System .err.println(
				 * "***** lag value is not missing!! (project missing lags)" );
				 * } count++; }
				 */
				...
				if (/* (i - firstLagIndex + 1) */lagNum > m_averageLagsAfter) {
					...
					for (int j = 1; j < m_numConsecutiveToAverage; j++) {
						if ((i + j) < insts.numAttributes()
								&& insts.attribute(i + j).name().startsWith("Lag_" + m_fieldsToLag.get(z))) {
	...
	private Instances createTimeIndexes(Instances insts) throws Exception {
		...
		if (m_timeStampName != null && m_timeStampName.length() > 0 && m_adjustForTrends) {
			...
			/*
			 * Instance first = insts.instance(insts.numInstances() - 1);
			 * Instance two = insts.instance(insts.numInstances() - 2);
			 * m_deltaTime = first.value(timeStampIndex) -
			 * two.value(timeStampIndex);
			 */
	...
	// any model coefficient computed for the timestamp is likely to be
	// extremely
	// small (appearing as 0 in output due to 4 decimal places precision).
	// Furthermore,
	// date timestamps with a periodicity of a month are not a constant number
	// of
	// milliseconds in length from one month to the next - remapping corrects
	// this
			...
			if (result.attribute(m_timeStampName).isDate()) {
				...
				for (int i = 0; i < result.numInstances(); i++) {
					if (!result.instance(i).isMissing(origIndex)) {
						if (m_dateBasedPeriodicityHandler.getPeriodicity() == Periodicity.MONTHLY
								|| m_dateBasedPeriodicityHandler.getPeriodicity() == Periodicity.WEEKLY
								|| m_dateBasedPeriodicityHandler.getPeriodicity() == Periodicity.QUARTERLY) {
				...
				for (int i = 0; i < result.numInstances(); i++) {
					...
					current = m_dateBasedPeriodicityHandler.remapDateTimeStamp(current, previous, m_timeStampName);
					previous = current;
					/*
					 * if (!current.isMissing(origIndex)) { if
					 * (m_dateBasedPeriodicity == Periodicity.MONTHLY ||
					 * m_dateBasedPeriodicity == Periodicity.WEEKLY ||
					 * m_dateBasedPeriodicity == Periodicity.QUARTERLY) { Date d
					 * = new Date((long)current.value(origIndex)); c.setTime(d);
					 * long year = c.get(Calendar.YEAR); long month =
					 * c.get(Calendar.MONTH); long week =
					 * c.get(Calendar.WEEK_OF_YEAR); long remapped = 0; if
					 * (m_dateBasedPeriodicity == Periodicity.MONTHLY) {
					 * remapped = ((year - m_dateTimeStampBase) * 12) + month; }
					 * else if (m_dateBasedPeriodicity == Periodicity.WEEKLY) {
					 * remapped = ((year - m_dateTimeStampBase) * 52) + week;
					 * 
					 * // adjust for the case where week 1 of the year actually
					 * starts // in the last week of December if (month ==
					 * Calendar.DECEMBER && week == 1) { remapped += 52; } }
					 * else if (m_dateBasedPeriodicity == Periodicity.QUARTERLY)
					 * { remapped = ((year - m_dateTimeStampBase) * 4) + ((month
					 * / 3L) + 1L); } current.setValue(current.numAttributes() -
					 * 1, (double)remapped); } else { double remapped =
					 * current.value(origIndex) - m_dateTimeStampBase; remapped
	...
	protected Instance remapDateTimeStamp(Instance inst) throws Exception {
		...
		if (m_addDateMap != null) {
			...
			result = m_dateBasedPeriodicityHandler.remapDateTimeStamp(result, null, m_timeStampName);

			/*
			 * int origIndex =
			 * result.dataset().attribute(m_timeStampName).index(); Calendar c =
			 * new GregorianCalendar();
			 * 
			 * if (!result.isMissing(origIndex)) { if (m_dateBasedPeriodicity ==
			 * Periodicity.MONTHLY || m_dateBasedPeriodicity ==
			 * Periodicity.WEEKLY || m_dateBasedPeriodicity ==
			 * Periodicity.QUARTERLY) { Date d = new
			 * Date((long)result.value(origIndex)); c.setTime(d); long year =
			 * c.get(Calendar.YEAR); long month = c.get(Calendar.MONTH); long
			 * week = c.get(Calendar.WEEK_OF_YEAR); long remapped = 0; if
			 * (m_dateBasedPeriodicity == Periodicity.MONTHLY) { remapped =
			 * ((year - m_dateTimeStampBase) * 12) + month; } else if
			 * (m_dateBasedPeriodicity == Periodicity.WEEKLY) { remapped =
			 * ((year - m_dateTimeStampBase) * 52) + week;
			 * 
			 * // adjust for the case where week 1 of the year actually starts
			 * // in the last week of December if (month == Calendar.DECEMBER &&
			 * week == 1) { remapped += 52; } } else if (m_dateBasedPeriodicity
			 * == Periodicity.QUARTERLY) { remapped = ((year -
			 * m_dateTimeStampBase) * 4) + ((month / 3L) + 1L); }
			 * result.setValue(result.numAttributes() - 1, (double)remapped); }
			 * else { double remapped = result.value(origIndex) -
	...
	public static class PeriodicityHandler implements Serializable {
		 ...
		 *            strings such as 'sat' or 'june', specific dates (with
		 *            optional format string) such as '2011-08-22@yyyy-MM-dd' or
		 *            integers (which get interpreted differently depending on
		 *            the periodicity)
		 * 
		 * @param dateFormat
		 *            a default date format to use for parsing dates
		 * @throws Exception
		 *             if an entry in the list is unparsable or unrecognized
		 */
		public void setSkipList(String aList, String dateFormat) throws Exception {
			if (aList != null && aList.length() > 0) {
				...
				for (String p : parts) {
					...
					if (m_handlerPeriodicity == Periodicity.UNKNOWN || m_handlerPeriodicity == Periodicity.HOURLY
							|| m_handlerPeriodicity == Periodicity.DAILY || m_handlerPeriodicity == Periodicity.MONTHLY) {
					...
					// last of all try as a specific date (if we have a date
					// formatting
					// string)
					if (dateFormat != null && dateFormat.length() > 0) {
						// first check to see if there is a custom format
						// attached to this
						// entry
						String datePart = p;
						if (p.indexOf('@') > 0) {
		 ...
		 * 
		 * @param tsbase
		 *            the first date time stamp value in the batch training data
		...
		public void setDateTimeStampInitial(long tsbase) {
			...
			if (m_handlerPeriodicity == Periodicity.MONTHLY || m_handlerPeriodicity == Periodicity.WEEKLY
		 ...
		 * @throws Exception
		 *             if the periodicity being managed is not date
		 *             timestamp-based
		 */
		 ...
		 * 
		 * @param tsfinal
		 *            the last date timestamp value in the batch training data
		 ...
		 * 
		 * @param isDateBased
		 *            true if the periodicity being managed is date
		 *            timestamp-based
		 */
		 ...
		 * 
		 * @param inst
		 *            the instance containing a date timestamp to be remapped
		 * @param previous
		...
		public Instance remapDateTimeStamp(Instance inst, Instance previous, String timeStampName) throws Exception {
			...
			if (!result.isMissing(origIndex)) {
				...
				if (m_skipList != null && m_skipList.size() > 0 && previous != null) {
					// check this instance's date time stamp against the skip
					// list -
					// our fundamental assumption (for the training data) is
					// that these
					// dates
					// are not actually
					// present in the data (i.e. sat and sun for stock market
					// data). If
					// they
					// are in the data (but with missing targets) then the
					// missing value
					// interpolation routine will have filled them in, which is
					// the wrong
					// thing to do if they are supposed to be skipped over
					if (dateInSkipList(d)) {
						throw new Exception("This instance contains a date time stamp that is "
								+ "a member of the skip list - skip list entries are not time "
								+ "units with respect to the model and should not be present : " + inst.toString());
					}

					if (!previous.isMissing(origIndex)) {
						if (result.value(origIndex) >= previous.value(origIndex)) {
							// compared to the previous date are we more than
							// one time unit
							// ahead?
							...
							while (start < end) {
								...
								if (start < end) {
									if (dateInSkipList(new Date((long) start))) {
										...
										// oh oh the difference between the
										// current and previous
										// instance
										// is more than one time step but the
										// intervening step(s)
										// are
							...
							// we have a problem here - data is not sorted in
							// ascending order
							// of the date time stamp!
							throw new Exception("The data does not seem to be sorted in ascending order "
									+ "of the date time stamp!");
						}
					}
				}
				...
				if (m_skipList != null && m_skipList.size() > 0 && previous == null) {
					...
					// check that this instance does not occur before the first
					// training
					// instance!!
					if (origValue < m_dateTimeStampInitialVal) {
						throw new Exception("The timestamp for this instance occurs before the "
								+ "timestamp of the first training instance!");
					}
					// can't prime/forecast for values that occurred before the
					// training
					// data.

					double end = result.value(origIndex);
					// first advance end until it is not in the skip list (this
					// won't
					// be needed for priming instances that are within the
					// training
					// date range), but might occur for closed-loop forecasting
					// when
					...
					if (end < m_dateTimeStampFinalVal) {
						// priming/forecasting within the range of the training
						// data -
						// will have to recompute all skips from the initial
						// training
						// time stamp up to this instance and not apply the
						// pre-computed
						// skip total for the full training period
						applyTrainingSkipAdjust = false;
						start = m_dateTimeStampInitialVal;
					} else {
						// priming/forecasting beyond the last training date
						// time stamp seen
				...
				if (m_handlerPeriodicity == Periodicity.MONTHLY || m_handlerPeriodicity == Periodicity.WEEKLY
					...
					// it might (or might not) make sense to take the floor
					// here. For
					// daily data
					// I have the feeling that data arithmetic (adding 1 to day
					// of the
					// year)
					// may actually add slightly more than
					// a day at certain times (to account for) leap
					// seconds/years
	 ...
	 * 
	 * @param insts
	 *            the instances to determine the periodicity from
	 * @param timeName
	 *            the name of the time stamp attribute
	 * @param userHint
	 *            a specific periodicity to defer to. The user should provide a
	 *            specific periodicity when the data has non-constant
	 *            differences in time between consecutive elements and a skip
	 *            list will be used to correct for this. Specifying UNKNOWN as
	 *            the periodicity here will result in the heuristic detection
	 *            routine being applied.
	...
	protected Instances setupDerivedPeriodics(Instances insts) throws Exception {
		...
		if (m_adjustForTrends && !m_useArtificialTimeIndex) {
			...
			m_dateBasedPeriodicityHandler = determinePeriodicity(insts, m_timeStampName, m_userHintPeriodicity);
			if (m_skipEntries != null && m_skipEntries.length() > 0) {
				m_dateBasedPeriodicityHandler.setSkipList(m_skipEntries, m_dateFormat);
			}
			...
			if (insts.attribute(m_timeStampName).isDate()) {
				...
				if (m_customPeriodics != null) {
					for (String name : m_customPeriodics.keySet()) {
						...
						// check to see if we should create a multi-label
						// nominal
						// attribute instead of a numeric binary attribute. If
						// all
						// test intervals have a non-null label then attribute
						// will
						// be nominal
						...
						for (int j = 0; j < insts.numInstances(); j++) {
							...
							if (!Utils.isMissingValue(current.value(primaryIndex)) && !Utils.isMissingValue(j)) {
								...
								if (candidateMap.get(key) == null) {
									...
									// check to see if this value is consistent
									// with what
									// we've seen previously
									String previous = candidateMap.get(key);
									if (!previous.equals(value)) {
										// we need one unique value of the
										// secondary to occur
										// in conjunction for each primary (e.g.
										// months of the year
										// and quarters - each month is
		...
		if (m_primaryPeriodicName != null && m_primaryPeriodicName.length() > 0) {
			...
			if (!Utils.isMissingValue(lastPeriodicIndex)) {
				...
				if (successor != null) {
					...
					// TODO
					// We can either set a missing value here if we don't have a
					// successor
					// in the map
					// or we can look at the order that the values are declared
					// in the
					// header for
					// the primary periodic sequence and assume that this order
					// is
					// correct.
	...
	public Instances getTransformedData(Instances insts) throws Exception {
		...
		if (m_adjustForTrends
				&& (m_timeStampName == null || m_timeStampName.length() == 0 || insts.attribute(m_timeStampName) == null)) {
			...
			// not occur immediately after the last training event. Since the
			// time
			// index is artificial, all we can do for future predictions is
			// assume
			// that the n instances provided to the primeForecaster() method
			// overlap
			// the last n instances of the training data and that future
			// predictions
	...
	public Instance processInstancePreview(Instance source, boolean incrementTime, boolean setAnyPeriodic)
			throws Exception {
	 ...
	 * 
	 * @param source
	 *            an instance in original format
	 * @param incrementTime
	 *            true if any time stamp value should be incremented based on
	 *            the time stamp value from the last instance seen and set in
	 *            the outputted instance
	 * @param setAnyPeriodic
	 *            true if any user-specified periodic value should be set in the
	 *            transformed instance based on the value from the last instance
	 *            seen.
	 * @return a transformed instance
	 * @throws Exception
	 *             if something goes wrong.
	 */
	public Instance processInstance(Instance source, boolean incrementTime, boolean setAnyPeriodic, boolean temporary)
			throws Exception {
		...
		if (m_artificialTimeMaker != null) {
			...
			// set the correct value here - it can't be done after the fact
			// because
			// of other filters that create the product of time and something
			// else.
			if (incrementTime) {
			...
			if (m_adjustForTrends) {
				...
				if (incrementTime) {
					...
					double newTime = weka.classifiers.timeseries.core.Utils.advanceSuppliedTimeValue(m_lastTimeValue,
							m_dateBasedPeriodicityHandler);

					// default to add the delta
					/*
					 * double newTime = m_lastTimeValue +
					 * m_dateBasedPeriodicity.deltaTime();//m_deltaTime; Date d
					 * = new Date((long)m_lastTimeValue); Calendar c = new
					 * GregorianCalendar(); c.setTime(d); if
					 * (m_dateBasedPeriodicity == Periodicity.MONTHLY) {
					 * c.add(Calendar.MONTH, 1); newTime =
					 * (double)c.getTimeInMillis(); } else if
					 * (m_dateBasedPeriodicity == Periodicity.WEEKLY) {
					 * c.add(Calendar.WEEK_OF_YEAR, 1); newTime =
					 * (double)c.getTimeInMillis(); } else if
					 * (m_dateBasedPeriodicity == Periodicity.QUARTERLY) {
					 * c.add(Calendar.MONTH, 3); newTime =
					 * (double)c.getTimeInMillis(); } else if
					 * (m_dateBasedPeriodicity == Periodicity.DAILY) {
					 * c.add(Calendar.DAY_OF_YEAR, 1); newTime =
					 ...
					 * else {
					 * System.err.println("*****WARNING missing time..."); }
					 */
					// }
				}
	...
	public double advanceSuppliedTimeValue(double valueToAdvance) {
		return weka.classifiers.timeseries.core.Utils.advanceSuppliedTimeValue(valueToAdvance,
				m_dateBasedPeriodicityHandler);
	}

	public double decrementSuppliedTimeValue(double valueToDecrement) {
		return weka.classifiers.timeseries.core.Utils.decrementSuppliedTimeValue(valueToDecrement,
				m_dateBasedPeriodicityHandler);
	}
}
