...
public class TSEvaluation {
	...
	/**
	 * If true, then the model will be rebuilt after each forecasting iteration
	 * on the test data. I.e. if N is the training data and t is the first
	 * testing data point, then then after forecasting t, the model will be
	 ...
	 * are one-step-ahead, the second holds those that are two-steps-ahead
	 * etc.).
	 */
	...
	/**
	 * A map (keyed by metric name) of metrics for the test data. The value for
	 * a key is a List (indexed by step) of the associated metric.
	 */
	 ...
	 *            the training data
	 * @param testSplitSize
	 *            the number or percentage of instances to hold out from the end
	 *            of the training data to be test data.
	 * @throws Exception
	 *             if a problem occurs.
	 */
	public TSEvaluation(Instances trainingData, double testSplitSize) throws Exception {
		...
		setEvaluationModules("MAE,RMSE"); // automatically adds an error module
											// too
	 ...
	 * 
	 * @param evalOnTraining
	 *            true if evaluation is to be performed on the training data
	 */
	 ...
	 * 
	 * @param evalOnTest
	 *            true if evalution is to be performed on the training data
	 */
	 ...
	 * instances to be presented to the forecaster before a forecast is
	 * requested
	 * 
	 * @param primeSize
	 *            the number of instances to prime with
	 */
	...
	/**
	 * Set whether evaluation for test data should begin by priming with the
	 * first x test data instances and then forecasting from step x + 1. This is
	 * the only option if there is no training data and a model has been
	 * deserialized from disk. If we have training data, and it occurs
	 * immediately before the test data in time, then we can prime with the last
	 * x instances from the training data.
	 * 
	 * @param p
	 ...
	 * first x instances from the test data and then forecasting from step x +
	 * 1. This is the only option if there is no training data and a model has
	 * been deserialized from disk. If we have training data, and it occurs
	 * immediately before the test data in time, then we can prime with the last
	 * x instances from the training data.
	 * 
	 * @return true if evaluation of the test data will begin by priming with
	 *         the first x instances from the test data.
	 */
	...
	/**
	 * Set whether the forecasting model should be rebuilt after each
	 * forecasting step on the test data using both the training data and test
	 * data up to the current instance.
	 * 
	 * @param r
	 *            true if the forecasting model should be rebuilt after each
	 *            forecasting step on the test data to take into account all
	 *            data up to the current point in time
	 */
	 ...
	 * 
	 * @param evalModNames
	 *            a comma-separated list of evaluation module names.
	 * @throws Exception
	 *             if there are unknown evaluation modules requested.
	 */
	...
	/*
	 * public void setEvaluationModules(List<TSEvalModule> modules) { // check
	 * for an error module for (TSEvalModule m : modules) {
	 * 
	 * }
	 * 
	 ...
	 * @throws Exception
	 *             if there are no predictions available for the training data
	 */
	public ErrorModule getPredictionsForTrainingData(int stepNumber) throws Exception {
		...
		if (stepNumber > m_predictionsForTrainingData.size()) {
			throw new Exception("Only predictions up to " + numSteps + (numSteps > 1 ? "steps" : "step")
					+ "-ahead are available");
		}
	 ...
	 * @return the stepNumber step ahead predictions for all targets
	 * @throws Exception
	 *             if there are no predictions available for the test data
	 */
	 ...
	 * 
	 * @param forecaster
	 *            the forecaster being evaluated
	 * @param source
	 ...
	 *             if something goes wrong during evaluation
	 */
	public void evaluateForecaster(TSForecaster forecaster, boolean buildModel, PrintStream... progress)
			throws Exception {
		...
		if (forecaster instanceof TSLagUser) {
			...
			trainMod = weka.classifiers.timeseries.core.Utils.replaceMissing(trainMod, lagMaker.getFieldsToLag(),
					lagMaker.getTimeStampField(), false, lagMaker.getPeriodicity(), lagMaker.getSkipEntries());

			if (trainMod.numInstances() != m_trainingData.numInstances()) {
				m_trainingData = trainMod;
			}

			// 如果要对测试数据进行评估，则对被hold out的预留数据进行缺失数据填充
			if (m_evaluateTestData) {
				...
				testMod = weka.classifiers.timeseries.core.Utils.replaceMissing(testMod, lagMaker.getFieldsToLag(),
						lagMaker.getTimeStampField(), false, lagMaker.getPeriodicity(), lagMaker.getSkipEntries(),
						m_missingTargetListTestSet, m_missingTimeStampListTestSet, m_missingTimeStampTestSetRows);
		...
		if (m_evaluateTrainingData) {
			...
			if (forecaster instanceof TSLagUser) {
				// if an artificial time stamp is being used, make sure it is
				// reset for
			...
			for (int i = 0; i < m_trainingData.numInstances(); i++) {
				...
				if (i < m_primeWindowSize) {
					...
					if (forecaster instanceof OverlayForecaster
						...
						// can only generate forecasts for remaining training
						// data that
					...
					/**
					 * 用刚生成的forecast数据更新m_predictionsForTrainingData，其中m_predictionsForTrainingData中的
					 * 每个ErrorModule都包含了针对某一个instance的实际数据、预测数据、权重、置信区间等信息
					 * */
					updateEvalModules(m_predictionsForTrainingData, m_metricsForTrainingData, forecast, i,
							m_trainingData);
		...
		/* && !m_evaluateTrainingData */) {
			...
			Instances primeData = new Instances(m_trainingData, m_trainingData.numInstances() - m_primeWindowSize,
					m_primeWindowSize);

			forecaster.primeForecaster(primeData);

			// if overlay data is being used then the only way we can make a
			// forecast
			// beyond the end of the training data is if we have a held-out test
			// set
			// to
				...
				if (m_testData != null) {
					...
					for (PrintStream p : progress) {
						p.println("WARNING: Unable to generate a future forecast beyond the end "
								+ "of the training data because there is no future overlay " + "data available.");
					}
				}
			} else {
		...
		if (m_evaluateTestData) {
			...
			if (m_trainingData == null || m_primeForTestDataWithTestData) {
				...
				if (predictionOffsetForTestData >= m_testData.numInstances()) {
					throw new Exception("Priming using test data requires more instances "
				...
				if (forecaster instanceof TSLagUser) {
					if (((TSLagUser) forecaster).getTSLagMaker().isUsingAnArtificialTimeIndex()) {
						...
						if (m_primeForTestDataWithTestData) {
							if (m_trainingData == null) {
								artificialTimeStampStart = ((TSLagUser) forecaster).getTSLagMaker()
										.getArtificialTimeStartValue();
								artificialTimeStampStart += m_primeWindowSize;
								((TSLagUser) forecaster).getTSLagMaker().setArtificialTimeStartValue(
										artificialTimeStampStart);
							} else {
								((TSLagUser) forecaster).getTSLagMaker().setArtificialTimeStartValue(
		...
		/* && !m_evaluateTestData */) {
			...
			if (m_primeWindowSize > m_testData.numInstances()) {
				...
				if (m_trainingData != null) {
					if (difference > m_trainingData.numInstances()) {
						...
						primeData = new Instances(m_trainingData, m_trainingData.numInstances() - difference,
								difference);
					}
				}
			...
			if (forecaster instanceof OverlayForecaster && ((OverlayForecaster) forecaster).isUsingOverlayData()) {
				...
				for (PrintStream p : progress) {
					p.println("WARNING: Unable to generate a future forecast beyond the end "
							+ "of the test data because there is no future overlay " + "data available.");
				}
			} else {
	...
	/**
	 * Construct a string containing all general and forecaster-specific
	 * options.
	 * 
	 * @param forecaster
	 *            the forecaster to use
	 * @param globalInfo
	 *            true if global information (about info) on the forecaster is
	...
	protected static String makeOptionString(TSForecaster forecaster, boolean globalInfo) {
		...
		optionsText.append("\tThe number of instances to prime the forecaster with "
	 ...
	 * 
	 * @param forecaster
	 *            the forecaster to get the global info for
	 * @return the global info (synopsis) for the classifier
	 * @throws Exception
	 *             if there is a problem reflecting on the forecaster
	 */
	...
	public static void evaluateForecaster(TSForecaster forecaster, String[] options) throws Exception {
		...
		if (loadForecasterName.length() > 0) {
			...
			ois.close();
			// TODO should we save the last primeWindowSize instances of the
			// training
			// data
		...
		if (testingFileName.length() == 0 && testSize == 0 && rebuildModelAfterEachStep) {
			throw new Exception("Can't rebuild the model after each forecasting step on"
					+ " the test data if there is no test data specified or no holdout" + " size specified!");
		}
		...
		if (graphTargets.size() > 1 && stepsToPlot.size() > 1) {
			throw new Exception("Can't specify multiple targets to plot and multiple steps. "
		...
		if (outputPredsForTarget != null && trainingData != null && !noEvalForTrainingData) {
			...
			System.out.println(eval.printPredictionsForTrainingData(
					"=== Predictions " + "for training data: " + outputPredsForTarget + " (" + stepNum
							+ (stepNum > 1 ? "-steps ahead)" : "-step ahead)") + " ===", outputPredsForTarget, stepNum,
					primeWindowSize));
		}

		if (outputPredsForTarget != null && (testData != null || testSize > 0)) {

			int instanceNumberOffset = (trainingData != null && testSize > 0) ? eval.getTrainingData().numInstances()
					: 0;
			...
			System.out.println(eval.printPredictionsForTestData(
					"=== Predictions for test data: " + outputPredsForTarget + " (" + stepNum
							+ (stepNum > 1 ? "-steps ahead)" : "-step ahead)") + " ===", outputPredsForTarget, stepNum,
					instanceNumberOffset));
		}
		...
		if (saveGraphFileName != null && trainingData != null && !noEvalForTrainingData) {
			if (graphTargets.size() >= 1 || (graphTargets.size() == 1 && stepsToPlot.size() == 1)) {
				JPanel result = eval.graphPredictionsForTargetsOnTraining(GraphDriver.getDefaultDriver(), forecaster,
						graphTargets, stepsToPlot.get(0), primeWindowSize);

				GraphDriver.getDefaultDriver().saveChartToFile(result, saveGraphFileName + "_train", 1000, 600);

			} else {
				JPanel result = eval.graphPredictionsForStepsOnTraining(GraphDriver.getDefaultDriver(), forecaster,
						graphTargets.get(0), stepsToPlot, primeWindowSize);
		...
		if (saveGraphFileName != null && (testData != null || testSize > 0)) {
			...
			if (graphTargets.size() > 1 || (graphTargets.size() == 1 && stepsToPlot.size() == 1)) {
				JPanel result = eval.graphPredictionsForTargetsOnTesting(GraphDriver.getDefaultDriver(), forecaster,
						graphTargets, stepsToPlot.get(0), instanceOffset);

				GraphDriver.getDefaultDriver().saveChartToFile(result, saveGraphFileName + "_test", 1000, 600);
			} else {
				JPanel result = eval.graphPredictionsForStepsOnTesting(GraphDriver.getDefaultDriver(), forecaster,
						graphTargets.get(0), stepsToPlot, instanceOffset);
		...
		if (graphFutureForecastS.length() > 0) {
			if (trainingData != null && !noEvalForTrainingData && eval.m_trainingFuture != null) {
				JPanel result = eval.graphFutureForecastOnTraining(GraphDriver.getDefaultDriver(), forecaster,
						AbstractForecaster.stringToList(forecaster.getFieldsToForecast()));
			...
			if (testData != null || testSize > 0 && eval.m_testFuture != null) {
				JPanel result = eval.graphFutureForecastOnTesting(GraphDriver.getDefaultDriver(), forecaster,
						AbstractForecaster.stringToList(forecaster.getFieldsToForecast()));
	 ...
	 * 
	 * @param forecaster
	 *            the forecaster used to generate the supplied predictions
	 * @param futureForecast
	 *            the future forecasted values as a list of list of
	 *            NumericPrediction objects. The outer list is indexed by step,
	 *            i.e. the first element contains the predictions for t+1, the
	 *            second for t+2, .... The inner list is indexed by target.
	 * @param insts
	 ...
	 *             if a problem occurs.
	 */
	protected String printFutureForecast(TSForecaster forecaster, List<List<NumericPrediction>> futureForecast,
	...
	/**
	 * Print the target values from the test data followed by the future
	 * forecast from the end of the test data.
	 * 
	 * @param forecaster
	 *            the forecaster in use
	...
	public String printFutureTrainingForecast(TSForecaster forecaster) throws Exception {
		...
		if (m_trainingData == null || m_trainingData.numInstances() == 0) {
			throw new Exception("Can't forecast beyond the end of the"
					+ " training instances because no training instances have been " + "supplied!");
		}
	 ...
	 * 
	 * @param driver
	 *            the graph driver to graph with
	 * @param forecaster
	 *            the forecaster in use
	 * @param targetNames
	 *            the names of the targets to graph
	 * @param preds
	 *            a list (indexed by step) of list (indexed by target) of
	 *            predictions
	 * @param history
	 *            Instances containing historical target values
	 * @return a JPanel containing the graph
	 * @throws Exception
	 *             if a problem occurs during graphing
	 */
	protected JPanel graphFutureForecast(GraphDriver driver, TSForecaster forecaster, List<String> targetNames,
	...
	/**
	 * Graph historical and future forecasted values using a graph driver for
	 * the training data.
	 * 
	 * @param driver
	 *            the graph driver to graph with
	 ...
	 *             if a problem occurs during graphing
	 */
	public JPanel graphFutureForecastOnTraining(GraphDriver driver, TSForecaster forecaster, List<String> targetNames)
			throws Exception {
		...
		throw new Exception("[TSEvaluation] can't graph future forecast for training "
	 ...
	 *             if a problem occurs during graphing
	 */
	public JPanel graphFutureForecastOnTesting(GraphDriver driver, TSForecaster forecaster, List<String> targetNames)
			throws Exception {
		...
		throw new Exception("[TSEvaluation] can't graph future forecast for test set "
				+ " because there are no future predicitons.");
	}
	 ...
	 *            the target to plot
	 * @param stepsToPlot
	 *            a list of steps at which to plot the predicted target values -
	 *            e.g. 3,4,5 would plot 3-step-ahead, 4-step-ahead and
	 *            5-step-ahead predictions
	 * @return a JPanel containing the graph
	 * @throws Exception
	 *             if a problem occurs during graphing
	 */
	public JPanel graphPredictionsForStepsOnTraining(GraphDriver driver, TSForecaster forecaster, String targetName,
			List<Integer> stepsToPlot, int instanceNumberOffset) throws Exception {

		JPanel result = driver.getGraphPanelSteps(forecaster, m_predictionsForTrainingData, targetName, stepsToPlot,
	 ...
	 *             if a problem occurs during graphing
	 */
	public JPanel graphPredictionsForStepsOnTesting(GraphDriver driver, TSForecaster forecaster, String targetName,
			List<Integer> stepsToPlot, int instanceNumberOffset) throws Exception {

		JPanel result = driver.getGraphPanelSteps(forecaster, m_predictionsForTestData, targetName, stepsToPlot,
	 ...
	 * @param graphStepNum
	 *            the step at which to plot the targets - e.g. 4 would result in
	 *            a graph of the 4-step-ahead predictions for the targets
	 * @return a JPanel containing the graph
	 * @throws Exception
	 *             if a problem occurs during graphing
	 */
	public JPanel graphPredictionsForTargetsOnTraining(GraphDriver driver, TSForecaster forecaster,
			List<String> graphTargets, int graphStepNum, int instanceNumberOffset) throws Exception {

		ErrorModule preds = m_predictionsForTrainingData.get(graphStepNum - 1);

		JPanel result = driver.getGraphPanelTargets(forecaster, preds, graphTargets, graphStepNum,
				instanceNumberOffset, getTrainingData());
	 ...
	 *             if a problem occurs during graphing
	 */
	public JPanel graphPredictionsForTargetsOnTesting(GraphDriver driver, TSForecaster forecaster,
			List<String> graphTargets, int graphStepNum, int primeWindowSize) throws Exception {

		ErrorModule preds = m_predictionsForTestData.get(graphStepNum - 1);

		JPanel result = driver.getGraphPanelTargets(forecaster, preds, graphTargets, graphStepNum, primeWindowSize,
				getTestData());
	...
	/**
	 * Print the predictions for a given target at a given step-ahead level on
	 * the training data.
	 * 
	 * @param title
	 ...
	 * @throws Exception
	 *             if the predictions can't be printed for some reason.
	 */
	...
	/**
	 * Print the predictions for a given target at a given step-ahead level from
	 * a given offset on the training data.
	 * 
	 * @param title
	 *            a title for the output
	 * @param targetName
	 ...
	 *            the step-ahead level - e.g. 3 would print the 3-step-ahead
	 *            predictions
	 * @param instanceNumberOffset
	 *            the offset from the start of the training data from which to
	 *            print actual and predicted values
	 * @return a String containing the predicted and actual values
	 * @throws Exception
	 *             if the predictions can't be printed for some reason.
	 */
	public String printPredictionsForTrainingData(String title, String targetName, int stepAhead,
	 ...
	 *             if the predictions can't be printed for some reason.
	 */
	public String printPredictionsForTestData(String title, String targetName, int stepAhead, int instanceNumberOffset)
			throws Exception {
	 ...
	 * 
	 * @param title
	 *            a title to use for the output
	 * @param preds
	 *            the predictions for a single target
	 * @param stepAhead
	 *            the step that the supplied predictions are at
	 * @param instanceNumberOffset
	 *            the offset tfrom the start of the data that was used to
	 *            generate the predictions that the predictions start at
	 * @return
	 */
	protected String printPredictions(String title, List<NumericPrediction> preds, int stepAhead,
	...
	public String toSummaryString() throws Exception {
		...
		for (TSEvalModule mod : m_evalModules) {
			if (mod.getDescription().length() + 2 > maxWidthForLeftCol) {
				maxWidthForLeftCol = mod.getDescription().length() + 2; // we'll
																		// indent
																		// these
																		// 2
																		// spaces
	...
	private String summaryMetrics(int maxWidthForLeftCol, int maxWidth, List<String> targetFieldNames,
		...
		/*
		 * for (String targetName : targetFieldNames) {
		 * temp.append(pad(targetName, " ", maxWidth, true)); }
		 */
		 ...
		 * = 0; i < m_horizon; i++) { int n =
		 * metricsToUse.get(tempKey).get(i).getN }
		 */
		...
		for (int i = 0; i < targetFieldNames.size(); i++) {
			...
			for (String key : keys) {
				...
				for (TSEvalModule m : metricForSteps) {
					...
					String result = (Utils.isMissingValue(metricForTargetI)) ? "N/A" : Utils.doubleToString(
							metricForTargetI, 4);
